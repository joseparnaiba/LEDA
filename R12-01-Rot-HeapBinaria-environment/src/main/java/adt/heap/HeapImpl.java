...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
    	if (position >= 0 && position <= this.index) {
        	...
        	if (left <= this.index && comparator.compare(this.heap[left], this.heap[smallest]) < 0) {
            	smallest = left;
        	}
        	if (right <= this.index && comparator.compare(this.heap[right], this.heap[smallest]) < 0) {
            	smallest = right;
        	}
	...
	public T[] heapsort(T[] array) {
    	...
    	T[] originalHeap = Arrays.copyOf(this.heap, this.heap.length);
        ...
        return Arrays.copyOf(this.heap, array.length);
    	} finally {
